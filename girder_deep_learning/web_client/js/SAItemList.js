...
wrap(ItemListWidget, 'render', function (render) {
    ...
    function addLargeImageDetails(item, container, parent, extraInfo) {
        var elem;
	var itemId = item.attributes.meta.SAItem;
        ...
        elem.append($('<img class="waiting"/>').attr(
            'deferred-src', apiRoot + '/item/' +
                itemId + '/tiles/thumbnail?width=160&height=100'));
        ...
        _.each(extra.images || [], function (imageName) {
            ...
            container.append(elem);
            elem.append($('<img class="waiting"/>').attr(
                'deferred-src', apiRoot + '/item/' + itemId +
    ...
    var hasAnyLargeImage = _.some(items, function (item) {
        return 'SAItem' in item.attributes.meta;
    });
    if (hasAnyLargeImage) {
        _.each(items, function (item) {
            var elem = $('<div class="large_image_container"/>');
            if ('SAItem' in item.attributes.meta) {
