# hard coded for dna damage images.
# save an image and a mask for training.
# a dot is places on the center of each rect.
...
def dump_item(gc, item_obj, out_path):
    ...
    # hard code a ROI to extract. (ymin, ymax, xmin, xmax)
    roi = {'left':0, 'top':1024, \
           'width':2048, 'height':1024}

    process_item_roi(gc, item_id, roi, file_path_root)
...
def print_usage():
    print("usage:")
    print("python %s serverName girder_id maskAnnotName level outpath"%sys.argv[0])
